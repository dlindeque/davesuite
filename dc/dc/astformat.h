...
namespace dc
{
    ...
    public:
        ...
        virtual auto visit(const CharRangesReAst* ast) -> void {
            ...
            public:
                ...
                virtual auto visit(const CharClassRangeAst* ast) -> void {
                    switch(ast->Cls) {
                        case CharClass::Alpha:
                            _stm << L"\\a";
                            break;
    ...
    public:
        ...
        virtual auto visit(const MatchDefinitionAst* ast) -> void {
            ...
            if (ast->IsReturn) {
                ...
                _stm << L" goto " << ast->GotoLabel;
            }
            _stm << L" -> " << ast->FunctionName << L' ' << ast->TokenName << L";" << std::endl;
        }
        virtual auto visit(const IncludeSetAst* ast) -> void {
            _stm << L"    include " << ast->Name;
            if (ast->IsReturn) {
                _stm << L" return";
            } else if (ast->IsGoto) {
                _stm << L" goto " << ast->GotoLabel;
    ...
    public:
        ...
        virtual auto visit(const AutomataAst* ast) -> void {
            _stm << L"  automata " << ast->Name << L" {" << std::endl;
            SetItemAstFormatter fmt(_stm);
            for(auto &si : ast->Items) {
                si->accept(&fmt);
            }
        ...
        virtual auto visit(const EnumAst* ast) -> void {
            _stm << L"  enum " << ast->Name << L" {" << std::endl;
            for(auto &item : ast->Items) {
                _stm << L"    " << item << L',' << std::endl;
            }
            _stm << L"  };" << std::endl;
        }
        virtual auto visit(const UsingNamespaceAst* ast) -> void {
            _stm << L"  using namespace " << ast->Name << L';' << std::endl;
        }
    };
    ...
    public:
        ...
        virtual auto visit(const StartItemAst* ast) -> void {
            _stm << L"set start = " << ast->Document << L";" << std::endl;
        }
        virtual auto visit(const NamespaceAst* ast) -> void {
            _stm << L"namespace " << ast->Name << L" {" << std::endl;
        ...
        virtual auto visit(const ImportAst* ast) -> void {
            _stm << L"import \"" << ast->Document << "\";" << std::endl;
        }
        virtual auto visit(const MixinAst* ast) -> void {
            _stm << L"mixin ...;" << std::endl;
        }
    };
}